...
Meteor.methods({
    ...
    createChatroom: function(chatroomAttributes) {
        ...
        //TODO: check that th hashtag is unique


        //TODO: move this function to the lib folder so that it can be shared
        //between the server and the client
        //chatroomAttributes.hashTag =  sanitizeHashtag(chatroomAttributes.hashTag);

        chatroomAttributes.isPrivate = Boolean(chatroomAttributes.isPrivate);
    ...
    deleteChatroom: function(chatroomId) {
        ...
        var chatroom = Chatrooms.findOne({_id: chatroomId});

        if (!chatroom) {
            bootbox.alert("Non existing chatroom id!", function() {});
            return;
        }


        // Double-checking the user is the owner of the chatroom
        if (userId === chatroom.ownerId ) {
            console.log('deleting the room: #' + chatroom);
            var updated = Chatrooms.remove({
                _id: chatroomId,
            });
        }
    ...
    togglePrivacy: function(chatroomId) {
        ...
        var chatroom = Chatrooms.findOne({_id: chatroomId});
        var privacyState = chatroom.isPrivate;

        // set the opposite state: true-->false, false-->true
        privacyState = !privacyState;

        if (!chatroom) {
            alert('Non existing chatroom id');
            return;
        }

        // Double-checking the user is the owner of the chatroom
        if (userId === chatroom.ownerId ) {
            //console.log('Updating the room: #' + chatroom);
            var updated = Chatrooms.update({
                _id: chatroomId,
            }, {
                $set: {
